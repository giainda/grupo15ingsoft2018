...
class ValidadorEditorViajeMultiple {
    private $id;
 ...
 public function __construct($id,$ciudad_origen,$ciudad_destino,$fecha_inicio,$duracion,$precio_total,$descripcion,$vehiculo,$conexion){
 ...
 private function validar_fecha_inicio($conexion,$fecha_inicio,$duracion){
    ...
    $fecha= new DateTime(date('Y-m-d H:i:s',strtotime($viaj->getFechaInicio())));
    ...
    if(isset($viajes)){
        foreach ($viajes as $viaje){
            ...
            $sumTime2 =date('Y-m-d H:i:s',strtotime('+'.$arr['hours'].' hour',strtotime($viaj->getFechaInicio())));
            ...
            $fecha2=new DateTime($viaje-> getFechaInicio()); 
             if($fecha >= $fecha2){
                 if($sumTime >= $fecha){
                    return "esta duracion no es posible, tiene otro viaje"; 
                 }
             }else{
                if($fecha <= $fecha2){
                 if($sumTime2 >= $fecha2){
        ...
        foreach($viajesAuto as $viaje ){
          ...
          $sumTime2 = new DateTime(date('Y-m-d H:i:s',strtotime('+'.$arr['minutes'].' minutes',strtotime($sumTime2))));
            ...
            $fecha2=new DateTime($viaje-> getFechaInicio()); 
             if($fecha >= $fecha2){
                 if($sumTime >= $fecha){
                    return "esta duracion no es posible, el auto tiene otro viaje"; 
                 }
             }else{
                if($fecha <= $fecha2){
                 if($sumTime2 >= $fecha2){
